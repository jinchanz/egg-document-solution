...
class YuQueViewer {
  ...
  bindRoute(yuqueClient, config) {
    ...
    this.unshiftRoute(`UpdateCacheByKey`, `/UPDATE-ABLULA-DOCUMENT-CACHE`, async (ctx, next) => {
      ...
      // 1. 更新具体 key 对应缓存
      cache.setex(key, { ...originData, data });
      // 2. 删除公共内容的缓存
      // 2.1 删除目录缓存
      cache.delete(`${baseUrl}/repos/${namespace}/toc`);
      // 2.2 删除文档列表缓存
      cache.delete(`${baseUrl}/repos/${namespace}/docs?include_hits=true`);
      // 2.3 删除关键词搜索缓存
      const searchKeys = cache.filterKeys('search');
